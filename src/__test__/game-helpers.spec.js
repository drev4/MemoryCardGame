...
it('generateCards should return an array of cards', () => {
  const cards = GameGenerator.generateCards();
  expect(Array.isArray(cards)).toBe(true);
});

it('shuffleNumbers should return an array of shuffled numbers', () => {
  const numbers = GameGenerator.shuffleNumbers();
  expect(Array.isArray(numbers)).toBe(true);
});

it('getNRandomNumbers should return an array of n random numbers', () => {
  const n = 3;
...
it('getCardStyle should return the CSS style for a card', () => {
